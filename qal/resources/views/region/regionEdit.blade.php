    ...
    <div class="card-body">
      <div class="row">
        <div class="col-7 table-responsive">
          ...
          <form action="{{ URL('/region-update') }}" method="POST" class="forms-sample">
            {{ csrf_field() }}    <!-- token -->
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-9">
                <select name="zoneId" id="zoneId" class="select2 form-control custom-select" style="width: 100%;" required="">
                      ...
                      <option value="{{ $allZone->id }}" @if($allZone->id==$edit->zoneid) selected="" @endif>{{ $allZone->name }} ({{ $allZone->code }})</option>
                      @endforeach
                  </select>
              </div>
            </div>
            <div class="form-group row">
              ...
              <div class="col-sm-9">
                <input type="text" class="form-control" id="regionName" name="regionName" placeholder="Region Name" value="{{ $edit->name }}" autocomplete="off" required="">
              </div>
            </div>
